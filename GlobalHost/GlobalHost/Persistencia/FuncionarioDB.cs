...
namespace GlobalHost.Persistencia
{
    class FuncionarioDB
    {
        ...
        public bool InsertSemDemissao(object obj)
        {
            bool result = false;
            ...
            {
                Funcionario fun = (Funcionario)obj;
                string SQL = @"INSERT INTO Funcionario(nome, dtnascimento, cpf, salario, endereco, dtadmissao telefone, email, login) 
                             VALUES (@nome, @dtnascimento, @cpf, @salario, @endereco, @dtadmissao, @telefone, @email, @login)";
                banco.Connect();
                result = banco.ExecuteNonQuery(SQL, "@nome", fun.Nome, "@dtnascimento", fun.Dtnascimento, "@cpf", fun.Cpf, "@salario", fun.Salario, "@endereco", fun.Endereco, "@dtadmissao", fun.Dtadmissao, "@telefone", fun.Telefone, "@email", fun.Email, "@login", fun.Login.Id);
                banco.Disconnect();
            }
            return result;
        }

        public bool UpadateSemDemissao(object obj)
        {
            bool result = false;
            ...
            {
                Funcionario fun = (Funcionario)obj;
                string SQL = @"UPDATE Funcionario SET nome = @nome, dtnascimento = @dtnascimento, cpf = @cpf, salario = @salario, endereco = @endereco, dtadmissao = @dtadmissao, dtdemissao = @dtdemissao, telefone = @telefone, email = @email, login = @login WHERE id = " + fun.Id;
                banco.Connect();
                result = banco.ExecuteNonQuery(SQL, "@nome", fun.Nome, "@dtnascimento", fun.Dtnascimento, "@cpf", fun.Cpf, "@salario", fun.Salario, "@endereco", fun.Endereco, "@dtadmissao", fun.Dtadmissao, "@telefone", fun.Telefone, "@email", fun.Email, "@login", fun.Login.Id);
                banco.Disconnect();
            }
